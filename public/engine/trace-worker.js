...
self.onmessage = async (ev) => {
  const { taskId, cropBlob, cropSize, pad, char, weight, fillIters, smooth, detail, widthScale, capRefPx, baselineOffset } = ev.data;
  try {
    const result = await traceGlyph(cropBlob, cropSize, pad, char, { weight, fillIters, smooth, detail, widthScale, capRefPx, baselineOffset });
