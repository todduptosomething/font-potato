...
const { PAGE, MARKER_SIZE, REG, GRID, HINT, HINT_WORD, BASELINE_Y_FRAC, CAP_Y_FRAC, XHEIGHT_Y_FRAC, layout, CHARSETS } = require('./templategeo');
...
function generateTemplatePDF({ charset = 'full' } = {}) {
  ...
  //
  // The cap and x-height guides above it are drawn DASHED and the baseline
  // SOLID, and that hierarchy is literal, not decorative: the baseline is the
  // one line the app actually reads, and the only one a letter must sit on.
  // The other two are suggestions a writer is free to overshoot — which
  // ascenders, descenders and swash flourishes all should. Dashes also lay
  // down less ink, so passing behind the black corner hint stays legible.
  ...
  for (const cell of L.cells) {
    ...
    const gx0 = x0 + w * 0.12, gx1 = x1 - w * 0.12;
    for (const frac of [CAP_Y_FRAC, XHEIGHT_Y_FRAC]) {
      const gy = y0 + h * frac;
      doc.moveTo(gx0, gy).lineTo(gx1, gy).lineWidth(0.4).dash(1.6, { space: 1.8 }).stroke(GHOST);
    }
    doc.undash();
    const baseY = y0 + h * BASELINE_Y_FRAC;
    doc.moveTo(gx0, baseY).lineTo(gx1, baseY).lineWidth(0.5).stroke(GHOST);
