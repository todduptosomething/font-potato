...
const { trace } = require('./trace2'); // detail-aware trace
 ...
 *   smooth  0..2 (curve roundness in potrace)
 *   slant   degrees -20..20 (italic shear; + leans right)
 *   width   0.6..1.4 (horizontal scale)
...
async function buildFont(dir, labels, opts, outDir) {
  ...
  // Detail slider 0..100: higher = MORE detail (fine). Low end fills gaps (blobby).
  const fineness = clamp(opts.detail == null ? 75 : opts.detail, 0, 100) / 100;
  const traceDetail = 1 - fineness;                 // trace2: 0 = fine .. 1 = simplify
  // 7 possible gap-fill amounts (6..0); bucket fineness into 7 EQUAL-width bins
  // so each amount gets the same slice of the slider. (A naive Math.round of a
  // 0..6 ramp bunches unevenly — e.g. the top two slider notches both landed
  // on 0 while others got 4 notches each. floor() into equal bins fixes that.)
  ...
  for (const e of entries) {
    ...
    if (fillIters > 0) {
      // close = dilate then erode: fills small gaps/breaks without net thickening
