  ...
  for (let it = 0; it < Math.abs(amount); it++) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const p = y * width + x;
        const nx = (x > 0 && ink[p - 1]) || (x < width - 1 && ink[p + 1]);
        if (grow && !ink[p] && nx) next[p] = 1;
        if (!grow && ink[p] && !((x > 0 && ink[p - 1]) && (x < width - 1 && ink[p + 1]))) next[p] = 0;
      }
    }
    ink.set(next);
  }
...
async function scaleWidthPreserveStroke(png, sx, pad) {
  ...
  const { data, info } = await sharp(png).grayscale()
    ...
    .resize(newInnerW, meta.height)
    .raw().toBuffer({ resolveWithObject: true });
  let ink = new Uint8Array(info.width * info.height);
  for (let i = 0; i < ink.length; i++) ink[i] = data[i] < 128 ? 1 : 0;

  const compensate = Math.round((strokeBefore * (1 - sx)) / 2);
  ink = xDilateErode(ink, info.width, info.height, compensate);

  const newWidth = info.width + 2 * pad;
  const out = Buffer.alloc(newWidth * meta.height, 255);
  for (let y = 0; y < meta.height; y++) {
    for (let x = 0; x < info.width; x++) {
      if (ink[y * info.width + x]) out[y * newWidth + (x + pad)] = 0;
    }
  }
 ...
 *   width       0.6..1.6 (horizontal scale, stroke-thickness-preserving; 1 = off)
...
async function buildFont(dir, labels, opts, outDir) {
  ...
  const width = clamp(opts.width == null ? 1 : opts.width, 0.6, 1.6);
