...
//
// Moving both upper lines down to 0.38 (cap) gives 0.38 of clear space above
// the cap line, slightly more than a full cap height, so a flourish has
...
//
// The x-height line sat exactly midway between them (0.56) for one pass —
// "evenly spaced" — but an even split implies lowercase reaching only half
// of cap height, which nobody's handwriting does. At Todd's direction it
// moved up to 0.51: the baseline-to-x-height band is now ~0.64 of the
// baseline-to-cap distance, the range real lowercase actually occupies, so
// writing to the guides no longer produces stunted lowercase.
//
