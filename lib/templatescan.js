...
async function scanTemplate(photo, dir, { charset = 'full' } = {}) {
  ...
  for (const cell of L.cells) {
    ...
    // Median-filter before thresholding. The template prints its guide letter
    // and baseline in light grey, and most printers render "light grey" as a
    // halftone dot pattern rather than a true continuous tone — visible at
    // photo resolution as a dither/hatch texture. Where that texture sits
    // right against real ink, individual dots cross the threshold and fuse
    // into the main stroke (despeckle can't remove them — they're not a
    // separate blob). A median filter erases dot-scale noise like this
    // without softening real edges the way a blur would.
    const grayU8 = Buffer.alloc(OUT_W * OUT_H);
    for (let i = 0; i < grayU8.length; i++) grayU8[i] = Math.round(cellGray[i]);
    const filtered = await sharp(grayU8, { raw: { width: OUT_W, height: OUT_H, channels: 1 } })
      .median(3).grayscale()
      .raw().toBuffer();
    mn = 255; mx = 0;
    for (let i = 0; i < filtered.length; i++) {
      cellGray[i] = filtered[i];
